...
def show_overview():
    ...
    # VI. Closing Calendar
    st.markdown("""
        <div style='background: linear-gradient(90deg, #3498db 0%, #2980b9 100%); padding: 15px; border-radius: 10px; margin-bottom: 20px;'>
            <h3 style='color: white; margin: 0; text-align: center; font-size: 1.8em; font-weight: 600;'>Closing Calendar</h3>
        </div>
    """, unsafe_allow_html=True)
    
    if 'Expected Close Date' in df.columns and 'Amount' in df.columns:
        # Convert Expected Close Date to datetime
        df['Expected Close Date'] = pd.to_datetime(df['Expected Close Date'], errors='coerce')
        
        # Filter out rows with invalid dates
        df = df.dropna(subset=['Expected Close Date'])
        
        # Create a pivot table for the heatmap
        df['Month'] = df['Expected Close Date'].dt.month
        df['Day'] = df['Expected Close Date'].dt.day
        
        # Calculate total amount for each date
        calendar_data = df.groupby(['Month', 'Day'])['Amount'].sum().reset_index()
        calendar_data['Amount'] = calendar_data['Amount'] / 100000  # Convert to Lakhs
        
        # Create a complete calendar grid
        all_days = pd.DataFrame({
            'Month': np.repeat(range(1, 13), 31),
            'Day': np.tile(range(1, 32), 12)
        })
        
        # Merge with actual data
        calendar_grid = all_days.merge(calendar_data, on=['Month', 'Day'], how='left')
        calendar_grid['Amount'] = calendar_grid['Amount'].fillna(0)
        
        # Create heatmap
        fig_calendar = go.Figure(data=go.Heatmap(
            z=calendar_grid.pivot(index='Month', columns='Day', values='Amount').values,
            x=range(1, 32),
            y=['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'],
            colorscale='Viridis',
            text=np.round(calendar_grid.pivot(index='Month', columns='Day', values='Amount').values, 1),
            texttemplate='₹%{text}L',
            textfont=dict(size=10, family='Segoe UI', weight='bold'),
            hoverongaps=False,
            hovertemplate='<b>%{y} %{x}</b><br>Amount: ₹%{z:.1f}L<extra></extra>',
            colorbar=dict(
                title='Amount (Lakhs)',
                titleside='right',
                tickformat='₹.1f',
        ...
        fig_calendar.update_layout(
            title=dict(
                text="Expected Deal Closings by Date",
                font=dict(size=22, family='Segoe UI', color='#2c3e50', weight='bold'),
                x=0.5,
                y=0.95,
                xanchor='center',
        ...
        with col1:
            total_closing_value = calendar_data['Amount'].sum()
            st.markdown(f"""
                <div style='text-align: center; padding: 15px; background: #f8f9fa; border-radius: 10px;'>
                    <div class='metric-label'>Total Closing Value</div>
                    <div class='metric-value'>₹{total_closing_value:,.1f}L</div>
                    <div style='color: #666; font-size: 0.9em;'>Expected to close</div>
                </div>
            """, unsafe_allow_html=True)
        
        with col2:
            avg_daily_value = calendar_data['Amount'].mean()
            st.markdown(f"""
                <div style='text-align: center; padding: 15px; background: #f8f9fa; border-radius: 10px;'>
                    <div class='metric-label'>Average Daily Value</div>
                    <div class='metric-value'>₹{avg_daily_value:,.1f}L</div>
                    <div style='color: #666; font-size: 0.9em;'>Per closing day</div>
                </div>
            """, unsafe_allow_html=True)
        
        with col3:
            max_daily_value = calendar_data['Amount'].max()
            max_date = calendar_data.loc[calendar_data['Amount'].idxmax()]
            st.markdown(f"""
                <div style='text-align: center; padding: 15px; background: #f8f9fa; border-radius: 10px;'>
                    <div class='metric-label'>Peak Closing Day</div>
                    <div class='metric-value'>₹{max_daily_value:,.1f}L</div>
                    <div style='color: #666; font-size: 0.9em;'>Day {max_date['Day']}, Month {max_date['Month']}</div>
                </div>
            """, unsafe_allow_html=True)
        
        # Add a detailed table view of upcoming closings
        st.markdown("### Upcoming Closings")
        upcoming_closings = df[df['Expected Close Date'] >= pd.Timestamp.now()].copy()
        upcoming_closings['Amount'] = upcoming_closings['Amount'] / 100000  # Convert to Lakhs
        upcoming_closings = upcoming_closings.sort_values('Expected Close Date')
        
        # Format the table
        upcoming_closings['Expected Close Date'] = upcoming_closings['Expected Close Date'].dt.strftime('%Y-%m-%d')
        upcoming_closings['Amount'] = upcoming_closings['Amount'].apply(lambda x: f"₹{x:,.1f}L")
        
        st.dataframe(
            upcoming_closings[['Expected Close Date', 'Amount', 'Sales Stage', 'Practice']],
            use_container_width=True
        )
    else:
        st.info("Required columns (Expected Close Date, Amount) not found in the dataset")
