...
def show_overview():
    ...
    if 'Status' in df.columns and 'Expected Close Date' in df.columns:
        ...
        # Month filter
        all_months = sorted(df['Month-Year'].unique(), key=lambda x: pd.to_datetime(x, format='%b %Y'))
        selected_month = st.selectbox(
            "Select Month",
            ["All Months"] + list(all_months),
            key="month_filter"
        )
        
        # Filter data based on selected month
        if selected_month != "All Months":
            df = df[df['Month-Year'] == selected_month]
        
        # Create mapping for cleaner labels
        df['Deal Category'] = df['Status'].fillna('Unknown')
        
        # Overall view
        st.markdown("#### Distribution for {}".format("All Months" if selected_month == "All Months" else selected_month))
        col1, col2 = st.columns(2)
        ...
        with col1:
            ...
            fig_deal_type = px.bar(
                ...
                x='Deal Category',
                y='Amount',
                title=f"Revenue by Deal Classification ({selected_month if selected_month != 'All Months' else 'Total'}) (in Lakhs)",
                color='Deal Category',
                color_discrete_sequence=['#4A90E2', '#45B7AF'],
        ...
        with col2:
            ...
            fig_deal_count = px.pie(
                ...
                title=f"Deal Distribution ({selected_month if selected_month != 'All Months' else 'Overall'})",
                color_discrete_sequence=['#4A90E2', '#45B7AF']
            )
            st.plotly_chart(fig_deal_count, use_container_width=True)
        
        # Summary metrics for selected month/overall
        col1, col2 = st.columns(2)
        with col1:
            committed_amount = df[df['Status'] == 'Committed for the Month']['Amount'].sum() / 100000
            st.metric(
                f"{'Monthly' if selected_month != 'All Months' else 'Total'} Committed Amount", 
                f"₹{committed_amount:,.2f}L"
            )
        with col2:
            upside_amount = df[df['Status'] == 'Upside for the Month']['Amount'].sum() / 100000
            st.metric(
                f"{'Monthly' if selected_month != 'All Months' else 'Total'} Upside Amount", 
                f"₹{upside_amount:,.2f}L"
            )
