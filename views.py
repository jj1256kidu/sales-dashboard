...
def show_sales_team_view(st):
    ...
    if len(filtered_df) > 0:
        ...
        with col1:
            current_week = st.selectbox("Select Current Week", weeks, index=len(weeks)-1, key="current_week_view")
        
        with col2:
            previous_week = st.selectbox("Select Previous Week", weeks, index=max(0, len(weeks)-2), key="previous_week_view")
        ...
        def calculate_metrics(week_data):
            committed = week_data[week_data['Probability_Num'] > 75]['Deal Value'].sum()
            upside = week_data[week_data['Probability_Num'].between(25, 75)]['Deal Value'].sum()
            closed_won = week_data[week_data['Status'] == 'Closed Won']['Deal Value'].sum()
